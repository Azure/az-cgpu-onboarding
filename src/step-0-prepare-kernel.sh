...
# Throws an error if both features are enabled
if [[ "$ENABLE_UBUNTU_PROPOSED_SOURCE_LIST" = "1" && "$ENABLE_UBUNTU_SNAPSHOT_SERVICE" = "1" ]]; then
...
enable_ubuntu_snapshot_service() {
    echo "Enabling Ubuntu snapshot service"
